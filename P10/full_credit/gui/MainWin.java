...
public class MainWin extends JFrame {
    ...
    protected void onCreateJavaClick() {  // Create a new Java product
        try {
            ...
            Object[] objects = {
             ...
             protected void onCreateOrderClick() {  // Create a new Java product
        try {
         
          //customer
            JLabel lCustomer = new JLabel("<HTML><BR/>Customer</HTML>");
            JComboBox<Object> dCustomer= new JComboBox<>(Customer.values());
            

            
            // Shots
            JLabel lServer = new JLabel("<HTML><BR/>Server</HTML>");
            JComboBox<Object> dServer= new JComboBox<>(Server.values());
            
            // This provides a scroll bar when a lot of customers or servers are added 
            JScrollPane scrollingCustomerPanel = new JScrollPane(CustomerPanel);
            scrollingCustomerPanel.setPreferredSize(new Dimension(200,80));
            
              JScrollPane scrollingServerPanel = new JScrollPane(ServerPanel);
            scrollingServerPanel.setPreferredSize(new Dimension(200,80));
    ...
    protected void onCreateServerClick() {
        ...
        final Pattern ssnFormat =Pattern.compile("^(\\d{3})-(\\d{3})-(\\d{4})$");
        while(true) {
            try {
                int button = JOptionPane.showConfirmDialog(
                    this,
                ...
                if(button == JOptionPane.OK_OPTION) {
                    ...
                    if(!(phoneFormat.matcher(phone).find()||ssnFormat.matcher(ssn).find()))
                        throw new IllegalArgumentException("Invalid Phone or Invalid ssn: " + phone + ssn);
                    store.addPerson(new Server(name, phone,ssn));
                    updateDisplay(Display.PEOPLE,Display.PRODUCTS,Display.ORDERS);
                }
                break;
            } catch(Exception e) {
    ...
    private JButton Products;          // Button to list products
    private JButton People;            // Button to list people
